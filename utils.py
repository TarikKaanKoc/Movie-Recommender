...
def preprocessing(df_mm, df_keywords, df_credits):
    ...
    df['tags'] = df['overview'] + ' ' + df['genres'] + ' ' + df['original_title'] + ' ' + df['keywords'] + ' ' + df[
        'cast']
...
def mb_preprocessing(movie_md, ratings):
    ...
    for temp in range(len(yt_url)):
        with st.container():
            ...
            with right_col:
                ...
                movie_genres = list(df[df["title"] == movie_name[temp]].genres)
